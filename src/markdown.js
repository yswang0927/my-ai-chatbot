    ...
    visit(tree, 'element', (node, index, parent) => {
      ...
      if (node.tagName === 'pre' && node.children[0]?.tagName === 'code') {
        ...
        const copyButton = h('button', {
          className: 'ai-chatbot-btn ai-chatbot-copy-btn ai-chatbot-code-copy-btn',
    ...
    visit(tree, 'element', (node, index, parent) => {
      ...
      if (node.tagName === 'pre' && node.children[0]?.tagName === 'code') {
        ...
        const previewButton = h('button', {
          className: 'ai-chatbot-btn ai-chatbot-mermaid-preview-btn',
          title: '预览图表',
          'data-variant': 'minimal'
        }, [h('span', { className: 'ai-chatbot-icon' }, [{ type: 'raw', value: eyeSvg }])]);

        const sourceButton = h('button', {
          className: 'ai-chatbot-btn ai-chatbot-mermaid-source-btn',
          title: '源码',
          'data-variant': 'minimal'
        }, [h('span', { className: 'ai-chatbot-icon' }, [{ type: 'raw', value: codeSvg }])]);

        const dwnButton = h('button', {
          className: 'ai-chatbot-btn ai-chatbot-mermaid-download-btn',
          title: '下载图表',
          'data-variant': 'minimal'
        }, [h('span', { className: 'ai-chatbot-icon' }, [{ type: 'raw', value: dwnSvg }])]);

        const copyButton = h('button', {
          className: 'ai-chatbot-btn ai-chatbot-copy-btn ai-chatbot-mermaid-copy-btn',
          title: '复制源码',
          'data-variant': 'minimal'
        }, [
          h('span', { className: 'ai-chatbot-icon ai-chatbot-copy-icon' }, [{ type: 'raw', value: copySvg }]),
        ...
        }, [h('div', { className: 'ai-chatbot-btn-group' }, [previewButton, sourceButton])]);

        const previewButtons = h('div', {
          className: 'ai-chatbot-btn-group ai-chatbot-mermaid-preview-btns'
        }, [dwnButton, zoomoutButton, zoominButton, fitViewportButton]);

        const sourceButtons = h('div', {
          className: 'ai-chatbot-btn-group ai-chatbot-mermaid-source-btns'
        }, [copyButton]);
