 ...
 * 缩放/平移插件
 * 
 * const pz = new PanZoom(containerDom, {
 *     transformElement: zoomDom,
 *     width: zoomDomWidth,
 *     height: zoomDomHeight,
 *     fitOnInit: true,
 *     beforeWheel: (e) => {
 *       // Ctrl + 滚轮缩放
...
export class PanZoom {
  constructor(selector, options) {
    ...
    if (typeof this.transformElement === 'string') {
      ...
      this.transformEl = this.transformElement;
    }
    if (!this.transformEl) {
  ...
  setTranformElement(selector) {
    if (!selector) {
      return;
    }

    if (typeof selector === 'string') {
      this.transformEl = document.querySelector(selector);
    } else {
      this.transformEl = selector;
    }
